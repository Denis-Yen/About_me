---
author: Denis Rodríguez
categories:
- Cursos
- Kobotoolbox
- Qgis
date: "2025-11-20"
draft: false
excerpt: Aprender a recolectar datos georreferenciados desde el celular y crear mapas claros y atractivos en QGIS,
  usando KoboToolbox y Kobocollect (android) para proyectos de campo, investigación y monitoreo.
layout: single
links:
- icon: door-open
  icon_pack: fas
  name: website
  url: https://sprightly-biscuit-258c7b.netlify.app/
- icon: hand-pointer
  icon_pack: fas
  name: Inscribirme Ahora
  url: https://github.com/apreshill/bakeoff
subtitle: Kobotoolbox + Qgis + Kobocollect
tags:
- hugo-site
title: Recopilación inteligente y mapeo SIG
---

![Forms_curso3](forms_curso3.png)

### ¿Estás listo para usar los SIG como una herramienta real para el desarrollo y el impacto social?. [(Proximamente)]()

---
### En que consiste el curso
En este curso completo, «Recopilación y mapeo de datos SIG con KoboToolbox y QGIS», aprenderás a transformar datos de campo en información territorial útil para la toma de decisiones.

El curso está diseñado para guiarte, paso a paso, por los fundamentos de los Sistemas de Información Geográfica (SIG) y su aplicación en investigación, 
gestión pública, planificación territorial y proyectos de desarrollo. Aprenderás a diseñar formularios de recolección, capturar datos georreferenciados en campo,
gestionar bases de datos espaciales y visualizar información mediante mapas claros y significativos, utilizando la combinación de KoboToolbox y QGIS.

{{< panelset class="greetings" >}}
{{< panel name="El Probelma! :rotating_light:" >}}
 
* Recolección y análisis SIG desconectados.
* Datos sin georreferenciación adecuada.
* Información en formatos dispersos.
* Dificultad para generar mapas útiles.
* Uso de equipos GPS costosos.

{{< /panel >}}
{{< panel name="La solución :white_check_mark:" >}}
Este curso te enseña a implementar un sistema completo de recolección y análisis estadístico de datos, donde:

* Flujo integrado: campo → SIG → mapas.
* Recolección con celular o tablet.
* Sin necesidad de GPS especializados.
* Formularios digitales con KoboToolbox.
* Análisis y mapas en QGIS.
* Datos listos para tomar decisiones..

{{< /panel >}}
{{< /panelset >}}


----
### Resultados que lograrás al finalizar el curso

- Comprender los **fundamentos de los Sistemas de Información Geográfica (SIG)** y reconocer sus principales aplicaciones en **salud pública**, **respuesta a desastres**, **planificación urbana**.

- Navegar y utilizar de manera eficiente las funciones de **KoboToolbox**, permitiéndole diseñar, gestionar y ejecutar procesos de **recopilación de datos digitales** de forma estructurada y segura.

- Aplicar el **proceso completo de creación, implementación y gestión de proyectos SIG**, integrando datos espaciales y no espaciales a partir de **estudios de casos reales**.

- Diseñar formularios digitales incorporando **lógica condicional, validaciones y controles de calidad**, garantizando la **consistencia y confiabilidad de los datos** recolectados en campo.

- Instalar y utilizar **QGIS**, reconociendo su interfaz, herramientas principales y funcionalidades clave para el análisis geoespacial.

- Trabajar con distintos **tipos de datos y fuentes SIG**, cargar mapas base y explorar información vectorial para su análisis e interpretación.

- Construir, editar, etiquetar y clasificar entidades geográficas, desarrollando habilidades de **comunicación visual clara e impactante** mediante mapas.

- Elaborar **mapas temáticos y diseños cartográficos**, incorporando elementos esenciales como **leyendas, barras de escala y flechas del norte**, de acuerdo con objetivos analíticos y comunicacionales.

- Exportar mapas en diversos formatos para su **presentación, difusión y uso en informes técnicos**.

- Sincronizar datos en **tiempo real** entre **KoboToolbox y QGIS** mediante el complemento **QRealTime**, facilitando la actualización continua de información geoespacial.

- Utilizar herramientas básicas de análisis espacial para **calcular áreas, distancias y seleccionar características**, apoyando la toma de decisiones basada en evidencia.


### 👤 Público objetivo
- **Tesistas** de licenciatura, maestría.
- **Investigadores** académicos  
- **Antropólogos** y **sociólogos** de campo  
- **Historiadores** y profesionales de ciencias sociales  
- Cualquier persona interesada en **recolectar, gestionar y analizar datos de forma profesional**


<span style="color:#6fc7da; font-weight:600;">
Convierte los datos de campo en hallazgos y conocimiento académico.
</span>

### Brouchure del curso
<!DOCTYPE html>
<html lang="es">
<head>
  <meta charset="UTF-8">
  <title>Inscripción al curso</title>
  <style>
    .btn-inscribirme {
      background-color: #0fffff;
      color: #000000;
      padding: 12px 24px;
      border-radius: 8px;
      font-weight: 600;
      text-decoration: none;
      display: inline-block;
      transition: background-color 0.0s ease, transform 0.0s ease;
    }

    .btn-inscribirme:hover {
      background-color: #000000;
      transform: translateY(-1px);
    }
  </style>
</head>
<body>

  <a href="https://drive.google.com/file/d/1WUFop1u3ql7EJJBJ74USgmR0Q3-WKeeB/view?usp=sharing" class="btn-inscribirme">
    📝 Click Aquí
  </a>

</body>
</html>